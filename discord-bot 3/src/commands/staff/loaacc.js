...
const { errorEmbed, infoEmbed, field, brandAuthor } = require('../../utils/embeds');
        ...
        return interaction.reply({
          embeds: [
            errorEmbed({
              ...
              description: 'Could not parse that duration.\n\nTry formats like `7d`, `12h`, `30m`, `2w`, or combos like `1d12h`.',
            }),
          ],
          ephemeral: true,
        });
        ...
        .send({
          embeds: [
            infoEmbed({
              ...
              description: `Your LOA in **${interaction.guild.name}** has been approved. Take the time you need — your role and standing are safe.`,
              thumbnail: interaction.guild.iconURL() ?? undefined,
              fields: [
                field('🛡️ Accepted By', interaction.user.tag, true),
                field('⏳ Duration', durationMs ? formatDuration(durationMs) : 'Indefinite', true),
                field('📝 Reason', row.reason, false),
              ],
            }),
          ],
        })
