...
const KNOWN_KEYS = [
  ...
  'loa_request_ping_role_id',
  'appeal_channel_id',
  'appeal_cooldown_days',
  'appeal_ping_role_id',
  'appeal_use_cards',
  'appeal_banner_pending',
  'appeal_banner_accepted',
  'appeal_banner_denied',
  'appeal_banner_anticheat',
];
  ...
  async execute(interaction) {
    ...
    if (sub === 'set') {
      ...
      return interaction.reply({
        embeds: [successEmbed({ title: '⚙️ Config Updated', description: `**${key}** set to \`${value}\`` })],
        ephemeral: true,
      });
    }
    ...
    if (sub === 'get') {
      ...
      return interaction.reply({
        embeds: [infoEmbed({ title: '⚙️ Config Value', description: `**${key}**: \`${value}\`` })],
        ephemeral: true,
      });
    }
    ...
    if (sub === 'list') {
      ...
      const description = rows.length
        ...
        : 'No config set yet.';
      return interaction.reply({
        embeds: [infoEmbed({ title: '⚙️ All Config Values', description })],
        ephemeral: true,
      });
    }
  },
