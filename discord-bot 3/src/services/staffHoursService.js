...
const { baseEmbed, goldEmbed, field, progressBar, brandAuthor, COLORS } = require('../utils/embeds');
...
async function runStaffHoursReport(client, guild) {
  ...
  const metText = met.length
    ? met.map((m) => `✅ <@${m.userId}> — ${progressBar(m.workedHours, m.requirement, 8)} · **${m.workedHours.toFixed(1)}**/${m.requirement}h`).join('\n')
    : '_Nobody met their requirement this period._';

  const notMetText = notMet.length
    ? notMet.map((m) => `❌ <@${m.userId}> — ${progressBar(m.workedHours, m.requirement, 8)} · **${m.workedHours.toFixed(1)}**/${m.requirement}h`).join('\n')
    : '_Everyone met their requirement!_ 🎉';
  ...
  await channel.send({
    ...
    embeds: [
      baseEmbed({
        color: notMet.length ? COLORS.WARNING : COLORS.SUCCESS,
        author: brandAuthor(guild, 'Staff Hours'),
        title: `📊 Staff Hours Report — Last ${periodDays} Day(s)`,
        description: `Summary of tracked duty time for this period.`,
        thumbnail: guild.iconURL() ?? undefined,
        fields: [
          field(`✅ Met Requirement — ${met.length}`, metText, false),
          field(`❌ Did Not Meet — ${notMet.length}`, notMetText, false),
        ],
      }),
    ],
  });
    ...
    await topChannel.send({
      embeds: [
        goldEmbed({
          ...
          description: `👑 <@${top.userId}> put in the most hours this period!`,
          thumbnail: guild.iconURL() ?? undefined,
          fields: [field('⏱️ Hours Worked', `**${top.workedHours.toFixed(1)}** hours`, true)],
        }),
      ],
    });
  }
