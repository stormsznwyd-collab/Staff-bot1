...
const { goldEmbed, infoEmbed } = require('../utils/embeds');
          ...
          .send({
            embeds: [
              infoEmbed({
                ...
                description: `Your leave of absence in **${guild.name}** has expired and your Staff LOA role has been removed. Welcome back!`,
              }),
            ],
          })
          .catch(() => {});
...
function buildLoaConfirmationEmbed(guild, member, acceptedBy, row) {
  const endText = row.end_time ? `<t:${Math.floor(row.end_time / 1000)}:F> (<t:${Math.floor(row.end_time / 1000)}:R>)` : 'Indefinite';
  return goldEmbed({
    title: 'Leave of Absence Accepted',
    description: [
      `**Member:** <@${member.id}>`,
      `**Accepted by:** <@${acceptedBy.id}>`,
      `**Ends:** ${endText}`,
