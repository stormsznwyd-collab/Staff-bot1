...
async function handleVoiceStateChange(oldState, newState) {
  ...
  try {
    await member.send({
      embeds: [
        errorEmbed({
          title: 'Duty Warning: You Left the Staff VC',
          description:
            'You are still clocked in but left the staff voice channel. Rejoin within **2 minutes** or you will be automatically clocked out.',
        }),
      ],
    });
  } catch {
        ...
        .send({
          embeds: [
            errorEmbed({
              ...
              description: "You didn't rejoin the staff voice channel in time, so you've been automatically clocked out.",
            }),
          ],
        })
        .catch(() => {});
